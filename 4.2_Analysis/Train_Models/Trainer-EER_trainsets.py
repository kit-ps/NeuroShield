#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 0 --train train_hardware_BioSemi > out1.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 1 --train train_hardware_Geodisi > out2.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 2 --train train_hardware_HydroCe > out3.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 0 --train train_unconnected_hardware_BioSemi > out4.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 1 --train train_unconnected_hardware_Geodisi > out5.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 1 --train train_unconnected_hardware_HydroCe > out6.log 2>&1 &
#python3 Trainer-EER_trainsets.py --chosen_loss SupConLoss --cuda_devices 0 --train train_raw_unconnected > out7.log 2>&1 &
...
class EEGDataset(Dataset):
    def __init__(self, path: str):
        ...
        self.subj2idx  = {s: i for i, s in enumerate(unique)}
...
class EEGDataset_valid(Dataset):
    ...
    def __init__(self, path: str):
        ...
        self._file    = h5py.File(path, 'r')
        # Data arrays
        self.data     = self._file['data']       # shape: (N, channels, samples) or similar
...
if __name__ == '__main__':
    train_ds = EEGDataset(os.path.join(data_dir, f"{feature}.h5"))
    val_ds   = EEGDataset_valid(os.path.join(data_dir, f"valid_raw.h5"))

    enroll_ds, verify_ds = make_enroll_verify_splits(val_ds, session_field='sessions')

    all_results = {}
    for name in models_to_run:
        conn = sqlite3.connect("eeg_studies_001.db", timeout=600)
        conn.execute("PRAGMA journal_mode=WAL;")
        conn.close()
        storage = RDBStorage(
            url="sqlite:///eeg_studies_001.db",
        ...
        for epoch in range(1, num_epochs+1):
            ...
            if val_loss < best_val_loss and epoch > 25:
                ...
                #torch.save(model.state_dict(),
                           #os.path.join('model_3', f"{name}_{chosen_loss}_{feature}_best.pth"))    
        writer.close()
        torch.save(model.state_dict(), os.path.join('model_3', f"{name}_{chosen_loss}_{feature}_final.pth"))

        os.makedirs('model_3', exist_ok=True)
        log_path = os.path.join('model', f"{name}_{chosen_loss}_log.csv")
        with open(log_path, 'w') as f:
            f.write("epoch,train_loss,val_loss\n")
            for e, tr, vl in log:
                f.write(f"{e},{tr:.6f},{vl:.6f}\n")

        del model, optimizer, scaler, writer, tr_loader, vl_loader
        torch.cuda.empty_cache()

    with open('best_hyperparams_00.json', 'w') as f:
